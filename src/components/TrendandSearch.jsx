...
function TrendandSearch(onTweetPosted) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const searchtag = await axios.get(
          `${BACKEND_URL}/api/hashtag/bulk?text=` + search
        );
        const sortedData = searchtag.data.data.sort(
          (a, b) => b.tweets.length - a.tweets.length
        );
      ...
      <div className="border rounded-xl mt-6 border-r-[0.5px] border-gray-700">
        <div className="p-1">
          ...
          {hashtagData.slice(0, 7).map((hashtag) => (
            <div
              className="mt-2 p-3 flex justify-between place-items-start hover:bg-gray-900"
              key={hashtag._id}
            >
              <div className=" ">
                <p className="text-sm  text-gray-500">
                  Entertainment · Trending .Post {hashtag.tweets.length}
                </p>
